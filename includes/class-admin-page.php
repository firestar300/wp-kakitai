...
class WP_Kakitai_Admin_Page {
	...
	public static function render_admin_page() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have sufficient permissions to access this page.', 'wp-kakitai' ) );
		}
		...
		<div class="wrap">
			...
			<div class="wp-kakitai-admin-container">
				<div class="card">
					<h2><?php esc_html_e( 'Dictionary Status', 'wp-kakitai' ); ?></h2>

					<table class="form-table">
						<tr>
							<th scope="row"><?php esc_html_e( 'Status', 'wp-kakitai' ); ?></th>
							<td>
								<?php if ( $status['installed'] ) : ?>
									<span class="dashicons dashicons-yes-alt" style="color: #46b450;"></span>
									<strong style="color: #46b450;">
										<?php esc_html_e( 'Installed', 'wp-kakitai' ); ?>
									</strong>
								<?php else : ?>
									<span class="dashicons dashicons-warning" style="color: #dc3232;"></span>
									<strong style="color: #dc3232;">
										<?php esc_html_e( 'Not Installed', 'wp-kakitai' ); ?>
									</strong>
								<?php endif; ?>
							</td>
						</tr>
						...
						<?php if ( $status['installed'] ) : ?>
							<tr>
								<th scope="row"><?php esc_html_e( 'Dictionary Version', 'wp-kakitai' ); ?></th>
								<td>
									<code><?php echo esc_html( $status['dict_version'] ); ?></code>
									<?php if ( $status['needs_update'] ) : ?>
										<span class="dashicons dashicons-update" style="color: #f0b849;" title="<?php esc_attr_e( 'Update available', 'wp-kakitai' ); ?>"></span>
									<?php endif; ?>
								</td>
							</tr>

							<tr>
								<th scope="row"><?php esc_html_e( 'Plugin Version', 'wp-kakitai' ); ?></th>
								<td><code><?php echo esc_html( $status['current_version'] ); ?></code></td>
							</tr>

							<tr>
								<th scope="row"><?php esc_html_e( 'Installed On', 'wp-kakitai' ); ?></th>
								<td>
									<?php
									echo esc_html(
										$status['install_date']
											? wp_date( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), $status['install_date'] )
											: __( 'Unknown', 'wp-kakitai' )
									);
									?>
								</td>
							</tr>
					...
					<div class="wp-kakitai-actions">
						<?php if ( ! $status['installed'] || $status['needs_update'] ) : ?>
							<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
								...
								<button type="submit" class="button button-primary">
									<?php
									if ( $status['installed'] ) {
										esc_html_e( 'Update Dictionaries', 'wp-kakitai' );
									} else {
										esc_html_e( 'Download & Install Dictionaries', 'wp-kakitai' );
									}
									?>
								</button>
								<p class="description">
						...
						<?php if ( $status['installed'] ) : ?>
							<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="margin-top: 20px;">
								...
								<button type="submit" class="button button-secondary" onclick="return confirm('<?php esc_attr_e( 'Are you sure you want to delete the dictionaries?', 'wp-kakitai' ); ?>');">
									<?php esc_html_e( 'Delete Dictionaries', 'wp-kakitai' ); ?>
								</button>
								<p class="description">
									<?php esc_html_e( 'Free up disk space by removing dictionary files.', 'wp-kakitai' ); ?>
								</p>
							</form>
						<?php endif; ?>
					</div>
				...
				<div class="card">
					...
					<p>
						...
						printf(
							...
							esc_html__( 'Version: %s', 'wp-kakitai' ),
					...
					<p>
						<a href="https://github.com/firestar300/wp-kakitai" target="_blank" rel="noopener noreferrer" class="button">
							<?php esc_html_e( 'View on GitHub', 'wp-kakitai' ); ?>
						</a>
					</p>
				</div>
			</div>
	...
	private static function render_notices() {
		if ( isset( $_GET['wp_kakitai_message'] ) ) {
			...
			$messages = array(
				'dict_installed'    => __( 'Dictionaries installed successfully!', 'wp-kakitai' ),
				'dict_updated'      => __( 'Dictionaries updated successfully!', 'wp-kakitai' ),
				'dict_deleted'      => __( 'Dictionaries deleted successfully!', 'wp-kakitai' ),
				'dict_install_fail' => __( 'Failed to install dictionaries. Please try again.', 'wp-kakitai' ),
				'dict_delete_fail'  => __( 'Failed to delete dictionaries.', 'wp-kakitai' ),
			);
	...
	public static function handle_install_dictionaries() {
		// Check permissions.
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have sufficient permissions to perform this action.', 'wp-kakitai' ) );
		}
		...
		if ( ! isset( $_POST['wp_kakitai_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['wp_kakitai_nonce'] ) ), 'wp_kakitai_install_dict' ) ) {
			wp_die( esc_html__( 'Security check failed.', 'wp-kakitai' ) );
		}
